...
contract TestMerkleRedeem is OwnableUpgradeable {
    ...
    function claimEpoch(
        ...
        require(!claimed[version][epoch][recipient], "Already claimed");
        require(verifyClaim(recipient, epoch, amount, merkleProof), "Incorrect merkle proof");

        claimed[version][epoch][recipient] = true;
    ...
    function claimEpochs(Claim[] memory claims) public {
        ...
        for (uint256 i = 0; i < claims.length; i++) {
            claim = claims[i];
            if (claimed[version][claim.epoch][recipient]) {
                continue;
            }
            ...
            claimed[version][claim.epoch][recipient] = true;
        }
    ...
    function claimStatus(
        ...
        for (uint256 i = 0; i < size; i++) {
            arr[i] = claimed[version][begin + i][account];
        }
        return arr;
    }
